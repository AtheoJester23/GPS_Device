...
{ 
  ...
  while (true) {
    ...
    {
      String jsonData = "{\"sensor\":\"temperature\",\"value\":24.5}"; // Example data
      PostFirebase("POST", FIREBASE_PATH, jsonData, &http_client);
    }
  }
}
...
void PostFirebase(const char* method, const String & path, const String & data, HttpClient* http)
{
  ...
  http->post(url, "application/json", data);  // Post request with JSON data
